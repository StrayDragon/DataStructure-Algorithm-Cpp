...
class TriNode {
 ...
 public:
  ...
  void setSmallData(E smallData) { smallData_ = smallData; }
  void setLargeData(E largeData) { largeData_ = largeData; }
  void setLChild(const Ptr& lChild) { lChild_ = lChild; }
  void setMChild(const Ptr& mChild) { mChild_ = mChild; }
  void setRChild(const Ptr& rChild) { rChild_ = rChild; }

  E getSmallData() const { return smallData_; }
  E getLargeData() const { return largeData_; }
  const Ptr& getLChild() const { return lChild_; }
  const Ptr& getMChild() const { return mChild_; }
  const Ptr& getRChild() const { return rChild_; }
};

template <typename E>
bool TriNode<E>::isLeft() const {
  return (this->lChild_ == nullptr && this->mChild_ == nullptr &&
          this->rChild_ == nullptr);
}

template <typename E>
bool TriNode<E>::isTwoChildren() const {
  return (this->lChild_ && this->mChild_ && this->rChild_ == nullptr) ||
