...
class AdjacencyMatrixGraph final : noncopyable, public Graph<VertexType, WeightType> {
  ...
  static AcyclicGraphTypes GetType() {
    return TYPE;
  }
  ...
  struct Edge {
    ...
    Edge(VertexType v1, VertexType v2, WeightType weight = 0) : v1_(v1), v2_(v2), weight_(weight) {}
  };
 ...
 public:
  ...
  bool add(VertexType startPosition, VertexType endPosition, WeightType edgeWight) override;

  bool remove(VertexType startPosition, VertexType endPosition) override;

  WeightType getEdgeWeight(VertexType startPosition, VertexType endPosition) const override;

  void depthFirstTraversal(VertexType startPosition, std::function<void(VertexType&)> visit) override;

  void breadthFirstTraversal(VertexType startPosition, std::function<void(VertexType&)> visit) override;
};
